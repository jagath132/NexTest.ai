...
const categoryStyles: Record<string, string> = {
  ...
  Validation: "bg-sky-500/10 text-sky-500 border-sky-500/20",
  "Validation checks": "bg-sky-500/10 text-sky-500 border-sky-500/20",
  Edge: "bg-amber-500/10 text-amber-500 border-amber-500/20",
  "Edge cases": "bg-amber-500/10 text-amber-500 border-amber-500/20",
};

const columnColors = ["badge-violet", "badge-rose", "badge-amber", "badge-cyan", "badge-primary", "badge-success"];
  ...
  async function exportPdf() {
    ...
    try {
      ...
      filteredAndSorted.forEach((tc, rowIdx) => {
        ...
        if (rowIdx % 2 === 0) { doc.setFillColor(248, 250, 252); doc.rect(margin, y, usableWidth, rowH, "F"); }
        x = margin;
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--accent)" }}>Test Case Table</p>
            <h3 className="mt-1 text-lg font-bold" style={{ color: "var(--text-primary)" }}>{title}</h3>
          </div>
          <div className="flex items-center gap-2">
            <select className="input-modern px-2 py-1.5 text-xs" value={filterCategory} onChange={(e) => setFilterCategory(e.target.value as TestCaseCategory | "all")}>
              <option value="all">All Categories</option>
              {allCategories.map((cat) => <option key={cat} value={cat}>{cat}</option>)}
            </select>
            <select className="input-modern px-2 py-1.5 text-xs" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as TestCaseStatus | "all")}>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
              <tr style={{ borderBottom: "2px solid var(--border-default)", background: "linear-gradient(90deg, var(--accent-soft), var(--accent-violet-soft), var(--accent-rose-soft))" }}>
                ...
                <th className="px-1 py-4 w-8" />
                {testCaseColumns.map((col, ci) => (
                  <th key={col.key} className={`px-5 py-4 text-xs font-semibold uppercase tracking-wider ${col.sortable ? "cursor-pointer select-none" : ""}`}
                    style={{ color: columnColors[ci % columnColors.length] === "badge-primary" ? "var(--accent)" : `var(--accent-${columnColors[ci % columnColors.length].replace("badge-", "")})` }}
            ...
            <tbody>
              {filteredAndSorted.map((testCase, rowIndex) => {
                ...
                return (
                  <tr key={testCase.tcId} className="transition-colors hover:bg-[var(--accent-soft)]"
                    style={{ background: rowIndex % 2 === 0 ? "var(--bg-secondary)" : "transparent" }}
                  >
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex rounded-full border px-2.5 py-0.5 text-xs font-semibold ${categoryStyles[testCase.category] ?? categoryStyles.Positive}`}>
                        {testCase.category}
                      </span>
                    </td>
          ...
          {filteredAndSorted.length === 0 && (
            <div className="p-8 text-center text-sm" style={{ color: "var(--text-muted)", background: "var(--accent-soft)" }}>
              <span className="badge badge-rose">No test cases match the current filters.</span>
            </div>
          )}
        </div>
      </div>
