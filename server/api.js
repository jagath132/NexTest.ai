...
import { storeBuildArtifact, getLatestBuild, saveWebhook, getWebhooks } from "./regression/builds.js";
import { createRun, listRuns, getRun, updateRun } from "./regression/runs.js";
import { authStore, authenticateToken, handleAuthRoute, decryptApiKey, findProductKeyByUserId, logAudit } from "./auth/index.js";

const ALLOWED_EXTENSIONS = [".apk", ".aab", ".zip", ".html", ".js", ".css", ".png", ".jpg", ".jpeg", ".svg", ".ico", ".json", ".txt", ".pdf", ".csv", ".xlsx", ".docx", ".pptx"];
    ...
    try {
      ...
      if (url.pathname.startsWith("/api/admin/")) {
        ...
        if (url.pathname === "/api/admin/users" && req.method === "GET") {
          ...
          sendJson(res, 200, {
            users: users.map((u) => ({ id: u._id.toString(), email: u.email, role: u.role || "Member", createdAt: u.createdAt, subscriptionTier: u.subscriptionTier || "free" })),
          });
          return;
        }
        ...
        if (url.pathname.match(/^\/api\/admin\/users\/([a-f0-9]+)$/) && req.method === "PUT") {
          ...
          await logAudit({ adminId: user.id, adminEmail: user.email, action: "update_role", resource: "user", resourceId: id, details: { role }, ip: clientIp });
        ...
        if (url.pathname === "/api/admin/unlock" && req.method === "POST") {
          ...
          await logAudit({ adminId: user.id, adminEmail: user.email, action: "unlock_account", resource: "user", details: { email }, ip: clientIp });
          sendJson(res, 200, { ok: true });
          return;
        }
        ...
        if (url.pathname.match(/^\/api\/admin\/users\/([a-f0-9]+)\/subscription$/) && req.method === "PUT") {
          const id = url.pathname.split("/").pop();
          const rawBody = await readRequestBody(req);
          ...
          await logAudit({ adminId: user.id, adminEmail: user.email, action: "update_subscription", resource: "user", resourceId: id, details: { tier, status }, ip: clientIp });
          sendJson(res, 200, { ok: true });
          return;
        }
      ...
      if (url.pathname === "/api/generate-test-cases" && req.method === "POST") {
        ...
        if (!apiKey) {
          sendJson(res, 500, { error: `Missing ${provider} API key. Provide it in settings or env.` });
          return;
        }
      ...
      if (url.pathname === "/api/generate-test-scripts" && req.method === "POST") {
        ...
        const selectedTestCases = Array.isArray(testCaseIds) && testCaseIds.length
      ...
      if (url.pathname === "/api/regression/generate" && req.method === "POST") {
        ...
        const result = await generateRegressionTestCases({ apiKey, provider, requirement, existingTestCases: testCases, platform: platform || "web", model: model || getDefaultModel(provider) });
      ...
      if (url.pathname === "/api/regression/scripts" && req.method === "POST") {
        ...
        const result = await generateRegressionScripts({ apiKey, provider, testCases, platform: platform || "web", framework: fw, language: lang, targetUrl: targetUrl || "http://localhost:3000", model: model || getDefaultModel(provider) });
      ...
      if (url.pathname.startsWith("/api/regression/runs/") && req.method === "POST" && url.pathname.endsWith("/execute")) {
        ...
        await updateRun(runId, { status: "passed", results, completedAt: new Date().toISOString() }, user.id);
      ...
      if (url.pathname === "/api/regression/builds/upload" && req.method === "POST") {
        ...
        upload.single("file")(req, res, async (error) => {
          if (error) { sendJson(res, 400, { error: error.message }); return; }
          try {
            const { platform, version } = req.body;
            if (!platform || !version) { sendJson(res, 400, { error: "platform and version are required." }); return; }
      ...
      if (url.pathname === "/api/regression/builds/webhook" && req.method === "POST") {
        ...
        if (!platform || !webhookUrl) { sendJson(res, 400, { error: "platform and url are required." }); return; }
      ...
      if (url.pathname === "/api/user/product-key" && req.method === "GET") {
        ...
        sendJson(res, 200, { productKey: keyDoc ? { key: keyDoc.key, activatedAt: keyDoc.usedAt } : null });
        return;
      }
      ...
      if (url.pathname === "/api/user/team" && req.method === "POST") {
        if (user.role !== "Admin") { sendJson(res, 403, { error: "Only admins can manage team." }); return; }
        const rawBody = await readRequestBody(req);
