      ...
      <div className="db-donut-layout">
        <div className="db-donut-chart">
          <svg width="110" height="110" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="40" fill="none" stroke="var(--color-elevated)" strokeWidth="10" />
            {segments.filter((s) => s.count > 0).map((s, i) => (
              <circle key={i} cx="50" cy="50" r="40" fill="none"
                ...
                style={{
                  transition: animate ? "stroke-dasharray 0.8s ease 0.3s, stroke-dashoffset 0.8s ease 0.3s" : "none",
                }}
              />
            ))}
          </svg>
          ...
          {data.map((d, i) => (
            <div key={d.plan} className="db-plan-row" style={{ animationDelay: `${i * 60}ms` }}>
              ...
              <div className="db-plan-bar-track">
                <div className="db-plan-bar-fill" style={{ width: `${(d.count / maxCount) * 100}%`, background: d.color }} />
              </div>
...
function PendingApprovalsWidget({ items, onNavigate, onApprove, onReject, loading }: {
  items: any[]; onNavigate?: (page: Page) => void; onApprove: (id: string) => void; onReject: (item: any) => void; loading: boolean;
}) {
  return (
      ...
      {items.length === 0 ? (
        ...
        <div className="db-pending-list">
          {items.slice(0, 3).map((item, i) => (
            <div key={item.pendingId} className="db-pending-item" style={{ animationDelay: `${i * 60}ms` }}>
              <div className="db-pending-avatar">{item.name ? item.name.charAt(0).toUpperCase() : "?"}</div>
...
function ActivityTimeline({ items, filter, onFilterChange }: {
  items: Activity[]; filter: string; onFilterChange: (f: string) => void;
}) {
  const filters = [
    { key: "all", label: "All" },
  ...
  return (
    <div className="db-card db-activity-card">
      ...
      <div className="db-pill-group" style={{ marginBottom: 16 }}>
        {filters.map((f) => (
          <button key={f.key} className={`db-pill${filter === f.key ? " active" : ""}`} onClick={() => onFilterChange(f.key)}>{f.label}</button>
        ))}
      </div>
      {filtered.length === 0 ? (
          ...
          {filtered.slice(0, 10).map((a, i) => (
            <div key={i} className="db-timeline-item" style={{ animationDelay: `${i * 50}ms` }}>
              <div className="db-timeline-dot-col">
                <div className={`db-timeline-dot ${a.type === "key_generated" ? "accent" : a.type === "key_used" || a.type === "approved" ? "success" : a.type === "registration" ? "warning" : "danger"}`} />
...
export function DashboardPage({ onNavigate }: { onNavigate?: (page: Page) => void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const rk = keysRes.data.keys.filter((k) => new Date(k.createdAt).getTime() > weekAgo).length;
        setRecentKeys(rk);
        setPrevWeekKeys(keysRes.data.keys.filter((k) => {
          const t = new Date(k.createdAt).getTime();
          return t > twoWeeks && t <= weekAgo;
        }).length);

        const rr = customersRes.data.customers.filter((c) => new Date(c.createdAt).getTime() > weekAgo).length;
        setRecentRegistrations(rr);
        setPrevWeekRegs(customersRes.data.customers.filter((c) => {
        ...
        for (const k of keysRes.data.keys.slice(0, 15)) {
          acts.push({ type: "key_generated", desc: `Key <strong>${k.key.slice(0, 8)}...</strong> generated`, time: new Date(k.createdAt) });
          if (k.usedAt) acts.push({ type: "key_used", desc: `Key <strong>${k.key.slice(0, 8)}...</strong> activated`, time: new Date(k.usedAt) });
        }
        for (const reg of allRegsRes.data.registrations.slice(0, 15)) {
          if (reg.status === "pending_verification") {
            acts.push({ type: "registration", desc: `<strong>${reg.email}</strong> registered for ${reg.plan || "a plan"}`, time: new Date(reg.createdAt) });
          } else if (reg.status === "ready") {
            acts.push({ type: "approved", desc: `<strong>${reg.email}</strong> approved`, time: new Date(reg.createdAt) });
          }
        }
  ...
  const planData = useMemo(() => {
    ...
    const colors: Record<string, string> = { Free: "var(--color-text-muted)", Pro: "var(--color-accent)", Enterprise: "var(--color-warning)" };
    return Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([plan, count]) => ({ plan, count, color: colors[plan] || "var(--color-text-muted)" }));
  }, [registrations]);

  const stats = keyStats || { total: 0, used: 0, available: 0, expired: 0 };
  const genTrend = prevWeekKeys > 0 ? Math.round(((recentKeys - prevWeekKeys) / prevWeekKeys) * 100) : recentKeys > 0 ? 100 : 0;
  ...
  const handleApprove = useCallback(async (pendingId: string) => {
    ...
    try {
      ...
      const res = await api.get<{ registrations: any[] }>("/api/admin/verifications?status=pending_verification");
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      await api.post("/api/admin/verifications/reject", { pendingId: rejectTarget.pendingId, reason: rejectReason });
      const res = await api.get<{ registrations: any[] }>("/api/admin/verifications?status=pending_verification");
      setPendingVerifications(res.data.registrations);
      setPendingCount(res.data.registrations.length);
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 460 }}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <textarea className="form-input" placeholder="e.g. Unable to verify identity..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={3} />
              </div>
            </div>
